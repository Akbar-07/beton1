...
useEffect(()=>{
    axios.get(`http://siriyuuc.beget.tech/auth/oneuser`,{headers:{Authorization : ` Bearer ${localStorage.getItem("token")}`}}).then(res=>{
        ...
        res.data.map(item=>{
           ...
           document.querySelector("#user-surname").value=item.surname 
           document.querySelector("#user-phone").value=item.phone 
           document.querySelector("#user-email").value=item.email 
        })
  ...
  function usermalumotozgartirish(){
    ...
    axios.put(`http://siriyuuc.beget.tech/auth/users/header/${data2[0].id}`,data,{headers:{Authorization : ` Bearer ${localStorage.getItem("token")}`}}).then(res=>{
